...
static void send_file(ftpvita_client_info_t *client, const char *path)
{
	...
	if ((fd = sceIoOpen(path, SCE_O_RDONLY, 0777)) >= 0) {
		...
		buffer = sceClibMspaceMalloc(clib_mspace, file_buf_size);
		if (buffer == NULL) {
		...
		sceIoClose(fd);
		sceClibMspaceFree(clib_mspace, buffer);
...
static int client_thread(SceSize args, void *argp)
{
	...
	sceClibMspaceFree(clib_mspace, client);
...
static int server_thread(SceSize args, void *argp)
{
	...
	while (1) {
		...
		if (client_sockfd >= 0) {
			...
			ftpvita_client_info_t *client = sceClibMspaceMalloc(clib_mspace, sizeof(*client));
...
int ftpvita_init(char *vita_ip, unsigned short int *vita_port)
{
	...
	if (ret == 0) {
		...
		net_memory = sceClibMspaceMalloc(clib_mspace, NET_INIT_SIZE);
...
error_netinit:
	if (net_memory) {
		sceClibMspaceFree(clib_mspace, net_memory);
		net_memory = NULL;
	}
error_netstat:
	return ret;
